  ...
  return (
    ...
    >
      ...
      <Card
        ...
        onClick={() => dispatch(openNoteDialog(props.note._id))}
      >
        {props.note.image && props.note.image !== '' && (
          <img src={props.note.image} className="w-full block" style={{ maxWidth: '200px'}} alt="note" />
        ...
        {props.note.tasks && props.note.tasks.length > 0 && (
          <ul className="px-20 my-16 flex flex-wrap list-reset">
            {props.note.tasks.map((item) => (
              <li key={item._id} className="flex items-center w-full">
