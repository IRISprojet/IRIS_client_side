...
function SignUpPage() {
  ...
  function onSubmit({ displayName, password, email }) {
    ...
    api
      ...
      .then((response) => {
        setIsLoading(false);
        dispatch(showMessage({ message: "Signing up was successful!" }));
        History.push(`/sign-up/complete/${response.data.accessToken}`);
      })
      .catch((error) => {
        setIsLoading(false);
        if (error.response.status === 400) {
          ...
          dispatch(
            showMessage({
              message:
                "Something went wrong when creating your account. Try again later!",
            })
          );
        }
      });
