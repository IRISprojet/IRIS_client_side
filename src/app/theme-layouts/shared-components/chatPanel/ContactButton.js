...
const ContactButton = ({ contact, selectedContactId, onClick }) => {
  return (
    <Root
      ...
      placement="left"
      active={selectedContactId === contact.user.id ? 1 : 0}
    >
      <Button
        onClick={() => onClick(contact?.user?.id)}
        className={clsx(
          "contactButton rounded-0 py-4 h-auto min-h-auto max-h-none",
          selectedContactId === contact?.user.id && "active"
        )}
      >
        <StyledStatus value={contact?.user?.status} />

        <Avatar src={contact?.user?.profilePicture} alt={contact?.user?.displayName}>
          {!contact?.user || contact?.user === "" ? contact?.user?.displayName : ""}
        </Avatar>
      </Button>
    </Root>
  );
